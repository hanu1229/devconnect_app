...
class _RatingViewState extends State<RatingView>{
  ...
  int dno = 0; // 나중에 개발자Info 페이지 에서 dno를 추출받기
  int cno = 0; // 나중에 기업Info 페이지 에서 cno를 추출받기
  final dio = Dio();
  ...
  Widget build(BuildContext context) {
    ...
    if( tokenrole == "Developer" ){
      ...
      menuTabs = CompanyMenuTabs(
        changePage: widget.changePage,
        selectedIndex: 7,
      );
    }else{
      menuTabs = SizedBox(); // 공백
    } // if end
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            menuTabs,
            SizedBox( height: 15 ,),
            Text("남긴 평가", style: TextStyle( fontSize: 18, fontWeight: FontWeight.bold, ), ),
            SizedBox( height: 10 ,),
            Expanded(
